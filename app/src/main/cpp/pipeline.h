...
public:
  ...
  // 新增：处理图片文件并返回识别结果
  std::vector<std::string> ProcessImageFile(const std::string& imagePath);
  
  // 新增：获取最后一次识别的结果
  std::vector<std::string> GetLastRecognitionResults();
  std::vector<float> GetLastRecognitionScores();
...
private:
  ...
  // 新增：存储最后一次识别结果
  std::vector<std::string> last_rec_text_;
  std::vector<float> last_rec_text_score_;
};
