// Copyright (c) 2019 PaddlePaddle Authors. All Rights Reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#include "Native.h"
#include "pipeline.h"
#include <android/log.h>

#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     com_baidu_paddle_lite_demo_ocr_db_crnn_Native
 * Method:    nativeInit
 * Signature:
 * (Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;ILjava/lang/String;)J
 */
JNIEXPORT jlong JNICALL
Java_com_lanqiDoctor_demo_ocr_Native_nativeInit(
    JNIEnv *env, jclass thiz, jstring jDetModelPath, jstring jClsModelPath,
    jstring jRecModelPath, jstring jConfigPath, jstring jLabelPath,
    jint cpuThreadNum, jstring jCPUPowerMode) {
  std::string detModelPath = jstring_to_cpp_string(env, jDetModelPath);
  std::string clsModelPath = jstring_to_cpp_string(env, jClsModelPath);
  std::string recModelPath = jstring_to_cpp_string(env, jRecModelPath);
  std::string configPath = jstring_to_cpp_string(env, jConfigPath);
  std::string labelPath = jstring_to_cpp_string(env, jLabelPath);
  std::string cpuPowerMode = jstring_to_cpp_string(env, jCPUPowerMode);

  return reinterpret_cast<jlong>(
      new Pipeline(detModelPath, clsModelPath, recModelPath, cpuPowerMode,
                   cpuThreadNum, configPath, labelPath));
}

/*
 * Class:     com_baidu_paddle_lite_demo_ocr_db_crnn_Native
 * Method:    nativeRelease
 * Signature: (J)Z
 */
JNIEXPORT jboolean JNICALL
Java_com_lanqiDoctor_demo_ocr_Native_nativeRelease(JNIEnv *env,
                                                                 jclass thiz,
                                                                 jlong ctx) {
  if (ctx == 0) {
    return JNI_FALSE;
  }
  Pipeline *pipeline = reinterpret_cast<Pipeline *>(ctx);
  delete pipeline;
  return JNI_TRUE;
}

/*
 * Class:     com_baidu_paddle_lite_demo_ocr_db_crnn_Native
 * Method:    nativeProcess
 * Signature: (JIIIILjava/lang/String;)Z
 */
JNIEXPORT jboolean JNICALL
Java_com_lanqiDoctor_demo_ocr_Native_nativeProcess(
    JNIEnv *env, jclass thiz, jlong ctx, jint inTextureId, jint outTextureId,
    jint textureWidth, jint textureHeight, jstring jsavedImagePath) {
  if (ctx == 0) {
    return JNI_FALSE;
  }
  std::string savedImagePath = jstring_to_cpp_string(env, jsavedImagePath);
  Pipeline *pipeline = reinterpret_cast<Pipeline *>(ctx);
  return pipeline->Process_val(inTextureId, outTextureId, textureWidth,
                               textureHeight, savedImagePath);
}

/*
 * Class:     com_lanqiDoctor_demo_ocr_Native
 * Method:    nativeProcessImageFile
 * Signature: (JLjava/lang/String;)[Ljava/lang/String;
 */
JNIEXPORT jobjectArray JNICALL
Java_com_lanqiDoctor_demo_ocr_Native_nativeProcessImageFile(
    JNIEnv *env, jclass thiz, jlong ctx, jstring jimagePath) {
  if (ctx == 0) {
    __android_log_print(ANDROID_LOG_ERROR, "OCR", "Pipeline context is null");
    return nullptr;
  }
  
  if (jimagePath == nullptr) {
    __android_log_print(ANDROID_LOG_ERROR, "OCR", "Image path is null");
    return nullptr;
  }
  
  std::string imagePath = jstring_to_cpp_string(env, jimagePath);
  __android_log_print(ANDROID_LOG_DEBUG, "OCR", "Processing image: %s", imagePath.c_str());
  
  Pipeline *pipeline = reinterpret_cast<Pipeline *>(ctx);
  if (pipeline == nullptr) {
    __android_log_print(ANDROID_LOG_ERROR, "OCR", "Pipeline pointer is null");
    return nullptr;
  }
  
  std::vector<std::string> results;
  try {
    results = pipeline->ProcessImageFile(imagePath);
  } catch (const std::exception& e) {
    __android_log_print(ANDROID_LOG_ERROR, "OCR", "ProcessImageFile failed: %s", e.what());
    return nullptr;
  } catch (...) {
    __android_log_print(ANDROID_LOG_ERROR, "OCR", "ProcessImageFile failed with unknown exception");
    return nullptr;
  }

  jobjectArray ret = env->NewObjectArray(
      results.size(), env->FindClass("java/lang/String"), env->NewStringUTF(""));
  for (int i = 0; i < results.size(); i++) {
    env->SetObjectArrayElement(ret, i, env->NewStringUTF(results[i].c_str()));
  }
  return ret;
}

/*
 * Class:     com_lanqiDoctor_demo_ocr_Native
 * Method:    nativeGetRecognitionResults
 * Signature: (J)[Ljava/lang/String;
 */
JNIEXPORT jobjectArray JNICALL
Java_com_lanqiDoctor_demo_ocr_Native_nativeGetRecognitionResults(
    JNIEnv *env, jclass thiz, jlong ctx) {
  if (ctx == 0) {
    return nullptr;
  }
  Pipeline *pipeline = reinterpret_cast<Pipeline *>(ctx);
  auto results = pipeline->GetLastRecognitionResults();

  jobjectArray ret = env->NewObjectArray(
      results.size(), env->FindClass("java/lang/String"), env->NewStringUTF(""));
  for (int i = 0; i < results.size(); i++) {
    env->SetObjectArrayElement(ret, i, env->NewStringUTF(results[i].c_str()));
  }
  return ret;
}

#ifdef __cplusplus
}
#endif
